...
const mapDispatch = (dispatch: any) => ({
  ...
  asyncConfirmation: dispatch.userModel.confirmationCode,
  asyncSignIn: dispatch.userModel.signIn,
})
...
const OnBoarding =  React.memo((props:Props) => {
  ...
  const { asyncConfirmation, asyncSignIn, history,location } = props

  const codeConfirmationHandler = async() => {
    // const { history, setUser, asyncGetProfile } = props
    if(formRef){
      const code = formRef!.current!.code.value
        
      try {
        const confirmedUser = await asyncConfirmation({username: location!.state!.username, code})
        console.log(confirmedUser)
        const user = await asyncSignIn(location.state.username, location.state.password)
        console.log(user)
        // setUser(user)
      ...
      <Grid item xs={12} sm={8} md={5} component={Paper} elevation={6} square>
        <div className={classes.paper}>
          ...
          <Button
            ...
            onClick={codeConfirmationHandler}
